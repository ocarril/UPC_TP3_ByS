...
CREATE TABLE [Trazabilidad].[LibroReceta](
	[codigolibroreceta] [varchar](10) NOT NULL,
	[fechaProducto] [date] NULL,
	[quimicoLaboratorista] [varchar](100) NULL,
	[codigoProducto] [varchar](10) NULL,
 CONSTRAINT [PK_LibroReceta] PRIMARY KEY CLUSTERED 
(
	[codigolibroreceta] ASC
)
)
ON [DATABASE01]
END
...
CREATE TABLE [Trazabilidad].[Procedimiento](
	...
	[actividadProcedimiento] [varchar](250) NULL,
	[plazoActividad] [varchar](250) NULL,
	[estado] [varchar](15) NULL,
	[tipoProcedimiento] [varchar](10) NULL,
...
/***************************************************************************************
* CREACION DE LA TABLA : Trazabilidad.RetiroProducto
* Tabla que contiene la informacion del Retiro de cada Producto.
****************************************************************************************/
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Trazabilidad.RetiroProducto') AND type in (N'U'))
BEGIN
CREATE TABLE [Trazabilidad].[RetiroProducto](
	[codigoProducto] [varchar](10) NOT NULL,
	[fecha] [date] NOT NULL,
...
/***************************************************************************************
* CREACION DE LA TABLA : Trazabilidad.TrazabilidadDetalle
* Tabla que contiene el detalle de la información de trazabilidad realizada a los productos
****************************************************************************************/
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Trazabilidad.TrazabilidadDetalle') AND type in (N'U'))
BEGIN
CREATE TABLE [Trazabilidad].[TrazabilidadDetalle](
	[codigoTraza] [varchar](10) NOT NULL,
...
IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_ProcedimientoById')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_ProcedimientoById] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_ProcedimientoById]
(
 @p_codProcedimiento		VARCHAR(10)=null
)
AS
BEGIN
SELECT 
	   P.codigoProcedimiento
      ,P.version
      ,P.fechIniVigencia
      ,P.fechFinVigencia
      ,P.responsable
      ,P.unidadPlazo
      ,P.observaciones
	  ,p.actividadProcedimiento
	  ,p.plazoActividad
	  ,p.tipoProcedimiento tipo
 FROM  [Trazabilidad].[Procedimiento]  P
 WHERE P.codigoProcedimiento=@p_codProcedimiento
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_U_Trazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_U_Trazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_U_Trazabilidad]
(
 @p_codigotraza	   varchar(10)
...
AS
BEGIN
	UPDATE [Trazabilidad].Trazabilidad SET 
	 [fechaTraza]=GETDATE(),[producto]=@p_producto,[descripcion]=@p_descripcion
	,[estado]=@p_estado 
	WHERE [codigoTraza]=@p_codigotraza
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_U_Procedimiento')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_U_Procedimiento] AS RETURN')
END
GO
...
,@p_fechIniVigencia      DATE
,@p_fechFinVigencia       DATE
,@p_responsable    varchar(50)
,@p_unidadPlazo        varchar(10)
,@p_observaciones       varchar(250)
,@p_actividadProcedimiento varchar(250)
,@p_plazoActividad      varchar(250)
,@tipo varchar(10)
)
AS
BEGIN
	UPDATE [Trazabilidad].[Procedimiento]
	   SET [version] = @p_version
	      ,[fechIniVigencia] = @p_fechIniVigencia
	      ,[fechFinVigencia] = @p_fechFinVigencia
	      ,[responsable] = @p_responsable
	      ,[unidadPlazo] = @p_unidadPlazo
	      ,[observaciones] = @p_observaciones
	      ,[actividadProcedimiento] = @p_actividadProcedimiento
	      ,[plazoActividad] = @p_plazoActividad
	      ,[estado] = 'ACTUALIZADO'
	      ,[tipoProcedimiento]=@tipo
	 WHERE [codigoProcedimiento] = @p_codigoProcedimiento
END
GO
...
IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_I_Trazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_I_Trazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_I_Trazabilidad]
(
 @p_codigoTraza					varchar(10) output
,@p_producto       				varchar(50)
,@p_descripcion           		varchar(255)
,@p_estado                  	varchar(50)
)
AS
BEGIN
	IF EXISTS(SELECT codigoTraza FROM Trazabilidad.Trazabilidad WHERE Producto=@p_producto AND estado='CONSULTADO')
	BEGIN
		UPDATE [Trazabilidad].Trazabilidad SET descripcion=@p_descripcion, fechaTraza=GETDATE()
		WHERE Producto=@p_producto AND estado='CONSULTADO'
		SELECT @p_codigoTraza=codigoTraza FROM Trazabilidad.Trazabilidad WHERE Producto=@p_producto AND estado='CONSULTADO'
	END
	ELSE
	BEGIN
		SELECT @p_codigoTraza = ISNULL(MAX(codigotraza),0)+1
		FROM Trazabilidad.Trazabilidad
		
		INSERT INTO [Trazabilidad].Trazabilidad
		([codigoTraza],[fechaTraza],[producto],[descripcion],[estado]) values
		(@p_codigoTraza,GETDATE(),@p_producto,@p_descripcion,@p_estado)		
	END
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_I_OrdenRetiro')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_I_OrdenRetiro] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_I_OrdenRetiro]
	@codigoProducto nVarchar(10), 
	@fecha datetime, 
	@autorizado nvarchar(50), 
	@motivo nvarchar(100), 
	@riesgo nvarchar(100)
AS
BEGIN
	INSERT INTO Trazabilidad.RetiroProducto (codigoProducto, fecha, autorizado, motivo, riesgo)
	Values(@codigoProducto, @fecha, @autorizado, @motivo, @riesgo)
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_I_InformeTrazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_I_InformeTrazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_I_InformeTrazabilidad]
(
 @p_codigoinformeTrazabilidad	varchar(10) output
,@p_producto       				varchar(50)
,@p_detalleAnalisis        		varchar(255)
,@p_anexo	                  	varchar(50)
,@p_codigoTraza					varchar(10)
)
AS
BEGIN
	--- ISNULL(MAX(codigoInformeTrazabilidad),0)+
	Declare @producto varchar(10), @estado varchar(50)
	
	Select @producto=producto, @estado=estado from Trazabilidad.Trazabilidad where codigoTraza =@p_codigoTraza
	
	SELECT @p_codigoinformeTrazabilidad =convert(varchar(2),@producto) +convert(varchar(8),GETDATE(),112) 
	---FROM Trazabilidad.InformeTrazabilidad
	
	if exists(Select [codigoInformeTrazabilidad] from [Trazabilidad].[InformeTrazabilidad] where [codigoInformeTrazabilidad]=@p_codigoinformeTrazabilidad)
	
	set @p_codigoinformeTrazabilidad=(select max(isnull([codigoInformeTrazabilidad],0))+1 from [Trazabilidad].[InformeTrazabilidad])
	
	INSERT INTO [Trazabilidad].[InformeTrazabilidad]
	           ([codigoInformeTrazabilidad]
	           ,[producto]
	           ,[detalleAnalisis]
	           ,[anexo],codigoTraza,estado) 
	values
	(@p_codigoinformeTrazabilidad,@p_producto,@p_detalleAnalisis,@p_anexo,@p_codigoTraza,'CREADO')
		
	UPDATE [Trazabilidad].[Trazabilidad] SET ESTADO='ACTUALIZADO'	WHERE codigoTraza =@p_codigoTraza	
END
GO
...
BEGIN
	...
	,pf.nombre
	,pf.posologia 
	,TP.actividadProcedimiento AS procedimiemtoDistribucion 
	,TP1.actividadProcedimiento AS procedimientoAlmacen
	,TP2.actividadProcedimiento AS procedimientoVenta
	,pf.quimicoFarmaceutico
	from Trazabilidad.FichaTecnicaProductoFarmacia pf
	LEFT JOIN Trazabilidad.Procedimiento TP ON TP.codigoProcedimiento=PF.procedimiemtoDistribucion  
	LEFT JOIN Trazabilidad.Procedimiento TP1 ON TP1.codigoProcedimiento=PF.procedimientoAlmacen  
	LEFT JOIN Trazabilidad.Procedimiento TP2 ON TP2.codigoProcedimiento=PF.procedimientoVenta
	WHERE 
	ISNULL(pf.codigoProducto,'')	=	isnull(@p_codProducto,'0')
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_InformeTrazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_InformeTrazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_InformeTrazabilidad]
(
 @p_codigoInformeTrazabilidad			VARCHAR(10)=null
)
AS
BEGIN
	SELECT
	...
	p.nombreProducto,
	IT.detalleAnalisis,
	convert(varchar(8),T.fechaTraza,112) fechaTrazastring,
	IT.codigoInformeTrazabilidad,
	IT.estado 	 	 
	FROM Trazabilidad.InformeTrazabilidad IT
	INNER JOIN Trazabilidad.Trazabilidad T ON IT.codigoTraza=t.codigoTraza
	INNER JOIN Trazabilidad.Producto P ON P.codigoProducto=T.producto
	WHERE IT.codigoInformeTrazabilidad= @p_codigoInformeTrazabilidad

	--SELECT
	--	T.codigoTraza,
	--	p.nombreProducto,
	--	IT.detalleAnalisis,
	--	convert(varchar(8),T.fechaTraza,112) fechaTraza,
	--	IT.codigoInformeTrazabilidad,
	--	IT.estado 	 	 
	--FROM Trazabilidad.InformeTrazabilidad IT
	--INNER JOIN Trazabilidad.Trazabilidad T ON IT.codigoTraza=t.codigoTraza
...
BEGIN
	...
	,quimicoFarmaceutico =@p_quimicoFarmaceutico        	
	,aprobar=@p_aprobar						
	WHERE codigoFichaTecProducto = @p_codigoFichaTecProducto
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_UsuarioTrazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_UsuarioTrazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_UsuarioTrazabilidad]
(
 @p_codCargo INT
,@p_codArea	INT
)
AS
BEGIN
	SELECT E.desLogin FROM [RecursosHumanos].[Empleado] E where  E.codArea=@p_codArea AND E.codCargo=@p_codCargo
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_Trazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_Trazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_Trazabilidad]
(
 @p_codProducto			VARCHAR(10)=null
) 
AS
BEGIN
	...
	P.nombreProducto,				
	T.ESTADO estado,
	ISNULL(IT.estado ,'')  estadoinformetrazabilidad
	FROM Trazabilidad.Trazabilidad T
	INNER JOIN Trazabilidad.Producto P ON P.codigoProducto=T.producto
	FULL OUTER JOIN Trazabilidad.InformeTrazabilidad IT ON IT.codigoTraza=T.codigoTraza
	WHERE (IT.estado='CREADO' OR T.estado='CONSULTADO' )AND
	ISNULL(T.producto,'')=(CASE WHEN ISNULL(@p_codProducto,'')<>''	
										 THEN  ISNULL(@p_codProducto,'') 
										 ELSE ISNULL(T.producto,'')	
									 END) 
									 ---T.ESTADO='CONSULTADO' AND
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_Procedimiento')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_Procedimiento] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_Procedimiento]
(
 @p_codProducto			VARCHAR(10)=null
)
AS
BEGIN
	...
	SELECT @nombreProducto=nombreProducto FROM Trazabilidad.Producto WHERE codigoProducto=@p_codProducto
	...
	SELECT 
		  ...
		  ,p.plazoActividad
	FROM  Trazabilidad.Procedimiento P
	LEFT JOIN [Trazabilidad].[FichaTecnicaProductoFarmacia] FT ON P.codigoProcedimiento=FT.procedimiemtoDistribucion
	LEFT JOIN [Trazabilidad].[FichaTecnicaProductoFarmacia] FT1 ON P.codigoProcedimiento=FT1.procedimientoAlmacen
	LEFT JOIN [Trazabilidad].[FichaTecnicaProductoFarmacia] FT2 ON P.codigoProcedimiento=FT2.procedimientoVenta 
	WHERE ( ISNULL(FT.codigoProducto,'')	=	
	    (CASE WHEN ISNULL(@p_codProducto,'')<>''	
		THEN  ISNULL(@p_codProducto,'') 
		ELSE ISNULL(FT.codigoProducto,'')END) ) OR( ISNULL(FT1.codigoProducto,'')	=	
	    (CASE WHEN ISNULL(@p_codProducto,'')<>''	
		THEN  ISNULL(@p_codProducto,'') 
		ELSE ISNULL(FT2.codigoProducto,'')END) ) OR( ISNULL(FT2.codigoProducto,'')	=	
	    (CASE WHEN ISNULL(@p_codProducto,'')<>''	
		THEN  ISNULL(@p_codProducto,'') 
		ELSE ISNULL(FT2.codigoProducto,'')END))
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_OrdendeCompraTrazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_OrdendeCompraTrazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_OrdendeCompraTrazabilidad]
(
 @p_codProducto			VARCHAR(10)=null 
,@p_codigoTraza			VARCHAR(10)=null
)
AS
BEGIN
	SELECT 
	...
	,oc.nombreProveedor 
	from Trazabilidad.OrdenDeCompra oc
	INNER JOIN Trazabilidad.Trazabilidaddetalle t on t.codigoCompra =oc.codigoCompra
	WHERE oc.codigoProducto=@p_codProducto and t.codigoTraza=@p_codigoTraza	
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_OrdenDeDespachoTrazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_OrdenDeDespachoTrazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_OrdenDeDespachoTrazabilidad]
(
  @p_codProducto			VARCHAR(10)=null
 ,@p_codigoTraza			VARCHAR(10)=null
)
...
BEGIN
	...
	,od.totalPedidos 
	from Trazabilidad.OrdenDeDespacho od
	INNER JOIN Trazabilidad.TrazabilidadDetalle t on t.numeroOrden=od.numeroOrden
	WHERE od.codigoProducto=@p_codProducto and t.codigoTraza=@p_codigoTraza	
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_U_TrazabilidadDetalle')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_U_TrazabilidadDetalle] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_U_TrazabilidadDetalle]
(
 @p_codigoTraza      			varchar(10)
,@p_codigoVenta         		varchar(10)
,@p_numeroKardex                varchar(10)
,@p_codigoCompra				varchar(10)
,@p_numeroOrden					varchar(10)
,@p_numeroHojaMerma				varchar(10)
,@p_nombreProducto				varchar(10)
,@p_codigoFichaTecProducto      varchar(10)
)
AS
BEGIN
	UPDATE [Trazabilidad].[TrazabilidadDetalle]
	   SET 
	       [codigoVenta] = @p_codigoVenta
	      ,[numeroKardex] = @p_numeroKardex
	      ,[codigoCompra] = @p_codigoCompra
	      ,[numeroOrden] = @p_numeroOrden
	      ,[numeroHojaMerma] = @p_numeroHojaMerma
...
IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_LibroRecetaTrazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_LibroRecetaTrazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_LibroRecetaTrazabilidad]
(
 @p_codProducto			VARCHAR(10)=null
,@p_codigoTraza			VARCHAR(10)=null
)
...
BEGIN
	...
	,lr.quimicoLaboratorista	
	from Trazabilidad.LibroReceta lr
	INNER JOIN Trazabilidad.TrazabilidadDetalle t on t.nombreProducto=lr.codigolibroreceta
	WHERE lr.codigoProducto=@p_codProducto and t.codigoTraza=@p_codigoTraza	
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_KardexTrazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_KardexTrazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_KardexTrazabilidad]
(
  @p_codProducto			VARCHAR(10)=null
 ,@p_codigoTraza			VARCHAR(10)=null
)
...
BEGIN
	...
	,k.salidas
	from Trazabilidad.Kardex k
	INNER JOIN Trazabilidad.TrazabilidadDetalle t on t.numeroKardex=k.numeroKardex
	WHERE k.codigoProducto=@p_codProducto and t.codigoTraza=@p_codigoTraza		
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_InformeVentaTrazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_InformeVentaTrazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_InformeVentaTrazabilidad]
(
  @p_codProducto			VARCHAR(10)=null
 ,@p_codigoTraza			VARCHAR(10)=null
)
...
BEGIN
	...
	,iv.nombreCliente	
	from Trazabilidad.InformeVenta iv	
	INNER JOIN Trazabilidad.TrazabilidadDetalle td on td.codigoVenta=iv.codigoVenta	
	WHERE iv.codigoProducto=@p_codProducto and td.codigoTraza=@p_codigoTraza	
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_S_HojaMermaTrazabilidad')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_S_HojaMermaTrazabilidad] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_S_HojaMermaTrazabilidad]
(
 @p_codProducto			VARCHAR(10)=null
,@p_codigoTraza			VARCHAR(10)=null
)
...
BEGIN
	...
	,hm.numeroHojaMerma	
	from Trazabilidad.HojaMerma hm
	INNER JOIN Trazabilidad.TrazabilidadDetalle td on td.numeroHojaMerma=hm.numeroHojaMerma
	WHERE hm.codigoProducto=@p_codProducto and td.codigoTraza=@p_codigoTraza	
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_I_TrazabilidadDetalle')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_I_TrazabilidadDetalle] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_I_TrazabilidadDetalle]
(
 @p_codigoTraza      			varchar(10)
,@p_codigoVenta         		varchar(10)
,@p_numeroKardex                varchar(10)
...
AS
BEGIN
	INSERT INTO [Trazabilidad].[TrazabilidadDetalle]
	           ([codigoTraza]
	           ,[codigoVenta]
	           ,[numeroKardex]
	           ,[codigoCompra]
	...
	VALUES
		( @p_codigoTraza,
		  ...
		  @p_numeroHojaMerma,
		  @p_nombreProducto,
		  @p_codigoFichaTecProducto)
END
GO

IF NOT EXISTS (SELECT NAME FROM sys.objects WHERE TYPE = 'P' AND NAME = 'pa_D_TrazabilidadDetalle')
BEGIN
	EXEC('CREATE PROCEDURE [Trazabilidad].[pa_D_TrazabilidadDetalle] AS RETURN')
END
GO

ALTER PROCEDURE [Trazabilidad].[pa_D_TrazabilidadDetalle]
(
 @p_codigoTraza      			varchar(10)
)
AS
BEGIN
	DELETE [Trazabilidad].[TrazabilidadDetalle] WHERE	codigotraza=@p_codigoTraza	
END
GO
...
/*************************************** Scripts de Trazabilidad ******************************************/
/**********************************************************************************************************/
/****** Object:  Table [Trazabilidad].[FichaTecnicaProductoProveedor]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[FichaTecnicaProductoProveedor] ([codigoFichaTecProveedor], [nombre], [nomTecnico], [modoUso], [contraIndicacion], [condicionesUso], [condicionesAlmacen], [condicionesComercial], [temperaturaMinima], [temperaturaMaxima], [laboratorioOrigen]) VALUES (N'1', N'APR LAB', N'LG', N'USO', NULL, NULL, NULL, NULL, NULL, N'10', NULL)
/****** Object:  Table [Trazabilidad].[LibroReceta]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[LibroReceta] ([codigolibroreceta], [fechaProducto], [quimicoLaboratorista], [codigoProducto]) VALUES (N'LR001', CAST(0xDB3A0B00 AS Date), N'RAUL CASTILLO', N'1')
/****** Object:  Table [Trazabilidad].[Kardex]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[Kardex] ([numeroKardex], [fecha], [ingreso], [salidas], [saldos], [observaciones], [codigoProducto]) VALUES (N'K001', CAST(0xDB3A0B00 AS Date), N'50', N'8', N'100', N'', N'1')
/****** Object:  Table [Trazabilidad].[InformeVenta]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[InformeVenta] ([codigoVenta], [fechaVenta], [nombreProducto], [cantidad], [nombreCliente], [precio], [codigoProducto]) VALUES (N'V001', CAST(0xDB3A0B00 AS Date), N'APRONAX', N'3', N'JUAN PEREZ', N'10', N'1')
/****** Object:  Table [Trazabilidad].[Trazabilidad]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[Trazabilidad] ([codigoTraza], [fechaTraza], [producto], [descripcion], [estado], [comentario]) VALUES (N'1', CAST(0x023B0B00 AS Date), N'1', NULL, N'ACTUALIZADO', NULL)
/****** Object:  Table [Trazabilidad].[RetiroProducto]    Script Date: 02/09/2016 17:43:00 ******/
/****** Object:  Table [Trazabilidad].[Producto]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[Producto] ([codigoProducto], [nombreProducto], [descripcion], [tipoProducto], [presentacion], [pesoProducto]) VALUES (N'1', N'APRONAX', N'APRONAX', N'ANTIBIOTICO', N'A', N'10')
INSERT [Trazabilidad].[Producto] ([codigoProducto], [nombreProducto], [descripcion], [tipoProducto], [presentacion], [pesoProducto]) VALUES (N'2', N'PANADOL', N'PANADOL ', N'ANTIBIOTICO', N'S', N'10')
INSERT [Trazabilidad].[Producto] ([codigoProducto], [nombreProducto], [descripcion], [tipoProducto], [presentacion], [pesoProducto]) VALUES (N'3', N'ANALGINA', N'ANALGINA', N'ANTIBIOTICO', N'S', N'15')
INSERT [Trazabilidad].[Producto] ([codigoProducto], [nombreProducto], [descripcion], [tipoProducto], [presentacion], [pesoProducto]) VALUES (N'4', N'AMOXICILINA', N'AMOXICILINA', N'ANTIBIOTICO', N'A', N'20')
INSERT [Trazabilidad].[Producto] ([codigoProducto], [nombreProducto], [descripcion], [tipoProducto], [presentacion], [pesoProducto]) VALUES (N'5', N'DICLOXICILINA', N'DICLOXICILINA', N'ANTIBIOTICO', N'EEE', N'25')
/****** Object:  Table [Trazabilidad].[Procedimiento]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[Procedimiento] ([codigoProcedimiento], [version], [fechIniVigencia], [fechFinVigencia], [responsable], [unidadPlazo], [observaciones], [actividadProcedimiento], [plazoActividad], [estado], [tipoProcedimiento]) VALUES (N'1', N'2.0', CAST(0x833B0B00 AS Date), CAST(0x4C3E0B00 AS Date), N'fffff', N'UN', N'dfds', N'ALMACEN 10 MESES', N'10 meses', N'ACTUALIZADO', N'2')
INSERT [Trazabilidad].[Procedimiento] ([codigoProcedimiento], [version], [fechIniVigencia], [fechFinVigencia], [responsable], [unidadPlazo], [observaciones], [actividadProcedimiento], [plazoActividad], [estado], [tipoProcedimiento]) VALUES (N'2', N'1.0', CAST(0x833B0B00 AS Date), CAST(0x683E0B00 AS Date), N'Rosa Rodriguez', N'UN', N'OBS', N'VENTA 8 MESES', N'8 meses', N'ACTUALIZADO', N'3')
INSERT [Trazabilidad].[Procedimiento] ([codigoProcedimiento], [version], [fechIniVigencia], [fechFinVigencia], [responsable], [unidadPlazo], [observaciones], [actividadProcedimiento], [plazoActividad], [estado], [tipoProcedimiento]) VALUES (N'3', N'2.0', CAST(0x813B0B00 AS Date), CAST(0x2E3E0B00 AS Date), N'Pedro Ramos', N'UN', N'OBS2', N'DISTRIBUCION 6 MESES', N'6meses', N'ACTUALIZADO', N'1')
/****** Object:  Table [Trazabilidad].[OrdenDeDespacho]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[OrdenDeDespacho] ([numeroOrden], [fecha], [totalPedidos], [pesoTotal], [observaciones], [codigoProducto]) VALUES (N'OD001', CAST(0xDB3A0B00 AS Date), N'5', N'300', NULL, N'1')
INSERT [Trazabilidad].[OrdenDeDespacho] ([numeroOrden], [fecha], [totalPedidos], [pesoTotal], [observaciones], [codigoProducto]) VALUES (N'OD002', CAST(0xDF3A0B00 AS Date), N'8', N'500', NULL, N'1')
/****** Object:  Table [Trazabilidad].[OrdenDeCompra]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[OrdenDeCompra] ([codigoCompra], [fechaCompra], [nombreProducto], [cantidad], [nombreProveedor], [costo], [codigoProducto]) VALUES (N'OC001', CAST(0xDB3A0B00 AS Date), N'APRONAX', N'50', N'LAB. ALFA', N'1000', N'1')
/****** Object:  Table [Trazabilidad].[InformeTrazabilidad]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[InformeTrazabilidad] ([codigoInformeTrazabilidad], [producto], [detalleAnalisis], [anexo], [codigoTraza], [estado]) VALUES (N'120160209', NULL, N'DSADSD', NULL, N'1', N'CREADO')
/****** Object:  Table [Trazabilidad].[HojaMerma]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[HojaMerma] ([numeroHojaMerma], [cantidadInsumo], [fecha], [motivo], [codigoProducto]) VALUES (N'M001', N'3', CAST(0xDB3A0B00 AS Date), N'EXPIRO', N'1')
/****** Object:  Table [Trazabilidad].[FichaTecnicaProductoFarmacia]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[FichaTecnicaProductoFarmacia] ([codigoFichaTecProducto], [nombre], [descripcion], [etiquetado], [procedimientoAlmacen], [procedimientoVenta], [procedimiemtoDistribucion], [posologia], [quimicoFarmaceutico], [aprobar], [codigoProcedimiento], [codigoFichaTecProveedor], [codigoProducto]) VALUES (N'1', N'APRONAX', N'APRONAX', N'NO', N'1', N'2', N'3', N'ANA PEREZ', N'MARIO', N'NO', N'1', N'1', N'1')
/****** Object:  Table [Trazabilidad].[TrazabilidadDetalle]    Script Date: 02/09/2016 17:43:00 ******/
INSERT [Trazabilidad].[TrazabilidadDetalle] ([codigoTraza], [codigoVenta], [numeroKardex], [codigoCompra], [numeroOrden], [numeroHojaMerma], [nombreProducto], [codigoFichaTecProducto]) VALUES (N'1', NULL, N'K001', NULL, NULL, NULL, NULL, NULL)
INSERT [Trazabilidad].[TrazabilidadDetalle] ([codigoTraza], [codigoVenta], [numeroKardex], [codigoCompra], [numeroOrden], [numeroHojaMerma], [nombreProducto], [codigoFichaTecProducto]) VALUES (N'1', NULL, NULL, N'OC001', NULL, NULL, NULL, NULL)
INSERT [Trazabilidad].[TrazabilidadDetalle] ([codigoTraza], [codigoVenta], [numeroKardex], [codigoCompra], [numeroOrden], [numeroHojaMerma], [nombreProducto], [codigoFichaTecProducto]) VALUES (N'1', N'V001', NULL, NULL, NULL, NULL, NULL, NULL)
INSERT [Trazabilidad].[TrazabilidadDetalle] ([codigoTraza], [codigoVenta], [numeroKardex], [codigoCompra], [numeroOrden], [numeroHojaMerma], [nombreProducto], [codigoFichaTecProducto]) VALUES (N'1', NULL, NULL, NULL, NULL, N'M001', NULL, NULL)
INSERT [Trazabilidad].[TrazabilidadDetalle] ([codigoTraza], [codigoVenta], [numeroKardex], [codigoCompra], [numeroOrden], [numeroHojaMerma], [nombreProducto], [codigoFichaTecProducto]) VALUES (N'1', NULL, NULL, NULL, NULL, NULL, N'LR001', NULL)
INSERT [Trazabilidad].[TrazabilidadDetalle] ([codigoTraza], [codigoVenta], [numeroKardex], [codigoCompra], [numeroOrden], [numeroHojaMerma], [nombreProducto], [codigoFichaTecProducto]) VALUES (N'1', NULL, NULL, NULL, N'OD001', NULL, NULL, NULL)
INSERT [Trazabilidad].[TrazabilidadDetalle] ([codigoTraza], [codigoVenta], [numeroKardex], [codigoCompra], [numeroOrden], [numeroHojaMerma], [nombreProducto], [codigoFichaTecProducto]) VALUES (N'1', NULL, NULL, NULL, N'OD002', NULL, NULL, NULL)
GO
